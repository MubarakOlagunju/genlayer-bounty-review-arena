...
class BountyFactory(gl.Contract):
    ...
    def evaluate_submission(self, bounty_id: int, submission_url: str) -> str:
        ...
        if result_json.get("meets_criteria") == True:
            # Finalize state first to prevent re-entrancy
            ...
            # Extract payout and zero out the escrow
            payout_amount = state["bounties"][bounty_id_str]["reward_escrow"]
            state["bounties"][bounty_id_str]["reward_escrow"] = 0
            
            self.factory_state = json.dumps(state)
            
            # Execute payout from specific bounty escrow
            gl.contract.transfer(sender, u256(payout_amount))
            
            return result_json_str

        # If rejected, state remains open and escrow remains locked
        return result_json_str
